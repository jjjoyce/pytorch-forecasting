...
class InterpretableMultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        attn = torch.stack(attns, dim=2) #(B,T,H,D)

        scor = torch.stack(scores, dim=1) if self.n_head > 1 else scores[0] #(B,H)

        #outputs = torch.mean(head, dim=2) if self.n_head > 1 else head
        if self.n_head > 1:
            scor = self.softmax(scor)
            scor = scor.unsqueeze(1).unsqueeze(3) # (B,1,H,1)
            scor = scor.permute(0,1,3,2) #(B,1,1,H)
            head = head.permute(0,1,3,2) #(B,T,D,H)
            outputs = torch.sum(head*scor,-1) #(B,T,D)
        else:
            outputs = head
